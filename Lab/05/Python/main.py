...
class MainWindow(QMainWindow):
    ...
    def encodeMsg(self):
        ...
        self.img_size = (self.img.width() * self.img.height() * 3) // 8 - len(self.marker)-self.masage_bytes
        if (self.img_size<self.byte_list_size):
        ...
        for i in range(index * 8,stop):
            x = (i // 3) % self.img.width()
            y = (i // 3) // self.img.width()
            shift = (int.from_bytes(byte_vector[i // 8], "big" )>> (7 - i % 8)) & 1
            rez = i % 3
            if (rez == 0):
                self.img.setPixel(x, y, (self.img.pixel(x, y) & ~0x010000) | (shift << 16))
            elif (rez == 1):
                self.img.setPixel(x, y, (self.img.pixel(x, y) & ~0x000100) | (shift << 8))
            elif rez == 2:
                self.img.setPixel(x, y, (self.img.pixel(x, y) & ~0x000001) | shift)
    ...
    def decodeMsg(self):
        ...
        count = 0
        index = 0
        last = (index + len(self.marker)+4) * 8

        for i in range (index * 8,last,1):
            count = (count << 1) | Bit(self.img, i)
            if (i % 8 == 7):
                vector_byte.push_back(bytes((count,)))
                count = 0
        ...
        last2 = (index2 + all_size) * 8


        for i in range(index2 * 8,last2):
            count2 = (count2 << 1) | Bit(self.img, i)

            if (i % 8 == 7):
                vector_byte2.push_back(bytes((count2,)))
                count2 = 0
